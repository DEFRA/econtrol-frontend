...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag econtrol-frontend`

Build:

```bash
docker build --target development --no-cache --tag econtrol-frontend:development .
```

Run:

```bash
docker run -p 3000:3000 econtrol-frontend:development
```
...
```bash
docker build --no-cache --tag econtrol-frontend .
```

Run:

```bash
docker run -p 3000:3000 econtrol-frontend
```
